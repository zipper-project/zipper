...
EC_KEY* SM2_private(const char *zHex) {
	EC_KEY	*eckey = NULL;
	BN_CTX *ctx = NULL;
	BIGNUM *p, *a, *b;
	EC_GROUP *group;
	...
	if (!BN_hex2bn(&x, "32C4AE2C1F1981195F9904466A39C9948FE30BBFF2660BE1715A4589334C74C7")) {
		fprintf(stdout, "BN_hex2bn x\n");
		goto err;
	};
	if (!EC_POINT_set_compressed_coordinates_GFp(group, P, x, 0, ctx)) {
		fprintf(stdout, "EC_POINT_set_compressed_coordinates_GFp\n");
		goto err;
	};
	// if (!EC_POINT_is_on_curve(group, P, ctx)) {
	// 	fprintf(stdout, "EC_POINT_is_on_curve\n");
	// 	goto err;
	// };
	if (!BN_hex2bn(&z, "FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFF7203DF6B21C6052B53BBF40939D54123")) {
		fprintf(stdout, "BN_hex2bn z\n");
		goto err;
	};
	if (!EC_GROUP_set_generator(group, P, z, BN_value_one())) {
		fprintf(stdout, "EC_GROUP_set_generator\n");
		goto err;
	};
	
	if (!EC_POINT_get_affine_coordinates_GFp(group, P, x, y, ctx)) {
		fprintf(stdout, "EC_POINT_get_affine_coordinates_GFp\n");
		goto err;
	}
	...
	if ((eckey = EC_KEY_new()) == NULL) {
		fprintf(stdout, "EC_KEY_new\n");
		goto err;
	}
	if (EC_KEY_set_group(eckey, group) == 0) {
		fprintf(stdout, "EC_KEY_set_group");
		goto err;
	}
	if (!BN_hex2bn(&z, zHex)) {
		fprintf(stdout, "BN_hex2bn z\n");
	...
	if (!EC_POINT_mul(group,P, z, NULL, NULL, ctx)) {
		fprintf(stdout, "EC_POINT_mul");
		goto err;
	}
	if (!EC_POINT_get_affine_coordinates_GFp(group,P, x, y, ctx)) {
		fprintf(stdout, "EC_POINT_get_affine_coordinates_GFp");
	...
	if (EC_KEY_set_private_key(eckey,z) == 0) {
		fprintf(stdout, "EC_KEY_set_private_key");
		goto err;
	}
	if (EC_KEY_set_public_key(eckey, P) == 0) {
		fprintf(stdout, "EC_KEY_set_public_key");
		goto err;
	}
	...
	// BNPrintf( y);
	// fprintf(stdout, "\n     z = 0x");
	// BNPrintf( EC_KEY_get0_private_key(eckey));
	// fprintf(stdout, "\n");
	return eckey;
err:
	EC_KEY_free(eckey);
...
EC_KEY* SM2_public(const char *xHex, const char *yHex) {
	EC_KEY	*eckey = NULL;
	BN_CTX *ctx = NULL;
	BIGNUM *p, *a, *b;
	EC_GROUP *group;
	...
	if (!BN_hex2bn(&y, yHex)) {
		fprintf(stdout, "BN_hex2bn y\n");
		goto err;
	}
	if (!EC_POINT_set_affine_coordinates_GFp(group,P, x, y, ctx)) {
		fprintf(stdout, "EC_POINT_set_affine_coordinates_GFp");
		goto err;
	}
	if (EC_KEY_set_public_key(eckey, P) == 0) {
		fprintf(stdout, "EC_KEY_set_public_key");
